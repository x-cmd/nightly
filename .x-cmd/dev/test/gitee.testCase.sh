...
test_gitee() {
    ...
    test_get_token() {
        # TODO:换成真实token
        ___x_cmd_gt_token "$(gitee)"
        assert "$(___x_cmd_gt_token)" = "$(gitee)"
    }
    test____x_cmd_gt_current() {
        assert "$(___x_cmd_gt_current owner)" = "mycw"
        assert "$(___x_cmd_gt_current type)" = "user"
    }
    test_get_token
    test____x_cmd_gt_current
    echo "gitee测试结束"
...
# user_key
test_user_key_id(){
    local user_key="$(___x_cmd_gt_user_key ls)"
    if [ $? -eq 1 ]; then
...
test_user_key(){
    ...
    local get_key="$(___x_cmd_gt_user_key get --id $id)"
...
[ $? -ne 0 ] && ___x_cmd_gt_log error "gt member ls ERROR"
...
[ $? -ne 0 ] && ___x_cmd_gt_log error "gt member del ERROR"[ $? -ne 0 ] && ___x_cmd_gt_log error "gt member add ERROR"


## enterprise
___x_cmd_gt_enterprise_test(){
    gt enterprise info  --admin 1> /dev/null 2>&1
    [ $? -ne 0 ] && ___x_cmd_gt_log error "gt enterprise info ERROR"
    gt enterprise info  mycw_yx 1> /dev/null 2>&1
    [ $? -ne 0 ] && ___x_cmd_gt_log error "gt enterprise info ERROR"
    gt enterprise repo ls  mycw_yx 1> /dev/null 2>&1
    [ $? -ne 0 ] && ___x_cmd_gt_log error "gt enterprise repo  ERROR"
    gt enterprise member ls --enterprise lteam18 1> /dev/null 2>&1
    [ $? -ne 0 ] && ___x_cmd_gt_log error "enterprise member ls  ERROR"
    gt enterprise member user mycw_yx zhengqbbb 1> /dev/null 2>&1
    [ $? -ne 0 ] && ___x_cmd_gt_log error "enterprise member user ERROR"
    gt enterprise member del --enterprise mycw_yx zhengqbbb 1> /dev/null 2>&1
    [ $? -ne 0 ] && ___x_cmd_gt_log error "enterprise member del ERROR"
    gt enterprise member add -e mycw_yx --role member  zhengqbbb 1> /dev/null 2>&1
    [ $? -ne 0 ] && ___x_cmd_gt_log error "enterprise member add ERROR"
    gt enterprise member access  -e mycw_yx --username zhengqbbb --role admin --name 秋彬 1> /dev/null 2>&1
    [ $? -ne 0 ] && ___x_cmd_gt_log error "enterprise member access ERROR"
    ___x_cmd_gt_log info "enterprise 测试完成"
    return 0
}

___x_cmd_gt_org_test(){
    gt org info 1> /dev/null 2>&1
    [ $? -ne 0 ] && ___x_cmd_gt_log error "org info ERROR"
    gt org info strong_task 1> /dev/null 2>&1
    [ $? -ne 0 ] && ___x_cmd_gt_log error "org info ERROR"
    gt org create x-bash-test 1> /dev/null 2>&1
    [ $? -ne 0 ] && ___x_cmd_gt_log error "org create ERROR" 
    gt org repo ls bash-gitee 1> /dev/null 2>&1
    [ $? -ne 0 ] && ___x_cmd_gt_log error "org repo ls ERROR" 
    gt org member ls bash-gitee 1> /dev/null 2>&1
    [ $? -ne 0 ] && ___x_cmd_gt_log error "org member ls ERROR" 
    gt org member user --username mycw bash-gitee 1> /dev/null 2>&1
    [ $? -ne 0 ] && ___x_cmd_gt_log error "org repo ls ERROR" 
    gt org member del --org bash-gitee zhengqbbb 1> /dev/null 2>&1
    [ $? -ne 0 ] && ___x_cmd_gt_log error "org member user ERROR" 
    gt org member add --org bash-gitee zhengqbbb 1> /dev/null 2>&1
    [ $? -ne 0 ] && ___x_cmd_gt_log error "org member add ERROR" 
    ___x_cmd_gt_log info "org 测试完成"
    return 0
}

___x_cmd_gt_issue_test(){
...
___x_cmd_gt_tag_test(){
    gt tag_releases --owner mycw_yx --repo x-bash --tag v1.0.0
}
# ___x_cmd_gt_repo_issue --owner mycw_yx --repo x-bash
# ___x_cmd_gt_issue_info --owner mycw_yx --repo x-bash --number I436Q2
# ___x_cmd_gt_issue_comments --owner mycw_yx --repo x-bash --number I436Q2
# ___x_cmd_gt_issue_logs --owner mycw_yx --number I436Q2
# ___x_cmd_gt_issue_comments_create --owner mycw_yx --repo x-bash --number I436Q2 --body "test 评论"
# ___x_cmd_gt_issue_comments_update --owner mycw_yx --repo x-bash --id 6079740 --body "test 评论1"
# ___x_cmd_gt_issue_comments_del --owner mycw_yx --repo x-bash --id 6079740

# ___x_cmd_gt_repo_release_create  --owner mycw_yx --repo x-bash --name test-release --body ... --tag_name v1.0.0
# ___x_cmd_gt_repo_release_update  --owner mycw_yx --repo x-bash --tag_name v1.0.0 --name test-update1 --body ... --id 151755
# ___x_cmd_gt_repo_release_latest --owner mycw_yx --repo x-bash
# ___x_cmd_gt_repo_release_delete --owner mycw_yx --repo x-bash  --id 151755
